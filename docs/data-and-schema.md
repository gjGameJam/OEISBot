...
| `data/runs/<A>-<YYYYmmdd-HHMMSS>-<sha8>.jsonl` | verification harness | every term a run emitted, one JSON object per line, as it arrived (a new term later voided by a list program's contract error stays `new` here; the attempt row counts it out). A term is written only once it has passed its index and value checks, so a run that had none accepted — including one whose very first term was wrong — leaves no file, and its attempt row names none |
...
| `extra` | JSON: `log` (run log path; **absent when no term was accepted**, because a term is logged only once it has passed its checks, so a run whose first term was wrong logs none — rows written before 2026-09-20 name it anyway, and that file was never created), `program` (the kept copy of a Python program as it ran, `<run log stem>.py`; Python only, and written whatever the run then does, so it is there even when `log` is not), `form` (`a(n)`, `predicate`, `print-loop`, `model`), `rewrites` (program notes), `weak_verification`, `extend_wall_s` and `mem_bytes` (the run's budgets, used by the dead-end check; absent on older rows, which therefore never count as `infeasible` or `extend_budget` dead ends), `attempt_call` (a key shared by every row of one `attempt_sequence` call, so the selection's failure penalty counts attempts, not rows: a random hex string, or `legacy-<first row id of the attempt>` on the 108 run rows written before 2026-09-19, set once from their grouping; skips have no `extra`), and `recheck` (only when new terms were found: the re-check note; while `recheck_pending`, `not done yet` or, after a failed re-check, `not done: <ErrorType>: <message>`) |
