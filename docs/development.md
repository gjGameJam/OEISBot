...
`python -m pytest` runs 274 tests in about 50 s. Tests that need the sandbox runtimes are marked
...
Counts are test functions; parametrized cases bring `test_pari.py` to 23, `test_codegen.py` to 84,
`test_attempt.py` to 52, `test_estimate.py` to 28, `test_model.py` to 10 and `test_recheck.py` to 9
collected tests.
...
| `test_verify.py` | 24 | the gate through the real sandbox: correct/extended, verify-only, offset error, shifted values, late wrong term, skipped index, crash, float values, early end, verify timeout, gmpy2 values, an infeasible stop ending the sandboxed run (no extension time, so no timing dependence; no new term, no censored time), over-prediction kill on a trusted projection, no kill on an untrustworthy one (where the old rule would have killed), no kill when the seconds per work unit climb 3× per term under a trustworthy cost fit (the rule before offer A would have killed), gp program; and the stderr tail in a detail (`_stderr_tail` keeping the last three lines that say something and dropping gp's caret rule, a line capped at `STDERR_LINE_CHARS`, `brief_detail`'s cut, a `crash` from a non-zero exit including the bare `: ` when stderr is empty, an `incomplete` with and without stderr, a real gp program calling an undefined function end to end, a stop reached with no terms keeping the tail, and a stop outside `_TAIL_STOPS` and one inside it with terms both keeping their detail) |
