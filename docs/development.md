...
`python -m pytest` runs 256 tests in about 50 s. Tests that need the sandbox runtimes are marked
...
One test is flaky: `test_verify.py::test_extension_stops_when_next_term_is_projected_infeasible` runs a
real doubling program for a 2 s extension and asserts on timing, so a slow moment on the machine can fail
it (about one run in four to eight, measured on 2026-09-19; see [status](status.md#open-offers-and-next-steps)).
Several sandbox tests that check kill thresholds are timing-sensitive too, but have passed repeatedly.

Counts are test functions; parametrized cases bring `test_pari.py` to 16, `test_codegen.py` to 83,
`test_attempt.py` to 51, `test_estimate.py` to 27, `test_model.py` to 10 and `test_recheck.py` to 9
collected tests.
